...
public class DrugTests3 {
  public static void main(String[] args) {
    ...
    // must be intialized. set to -1
    int weekBefore = -1;
    for (int i = 0; i < numTest; i++) {
      int selected = (int) (Math.random() * numEmployees) + 1;
      if(selected == weekBefore){
        System.out.printf("\nEmployee %d was picked last week. Selecting a new employee. ",selected);
        selected = (int) (Math.random() * numEmployees) + 1;
